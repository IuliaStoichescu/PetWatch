...
#define ACTIVITY_THRESHOLD_SLEEP 0.020
#define ACTIVITY_THRESHOLD_WALK 0.2
#define ACTIVITY_THRESHOLD_RUN 0.25

#define FALL_THRESHOLD 0.8
#define IMPACT_THRESHOLD 1.35
#define STATIONARY_AFTER_FALL 0.3
#define FALL_DETECTION_WINDOW 1000
...
// Calibration values
float accelCalibrationOffset = 0.0;
...
const unsigned long GPS_STANDBY_TIMEOUT = 120000;
const unsigned long GPS_WAKEUP_INTERVAL = 300000;
...
void sendGPSData() {
  ...
  String gpsData = String(MSG_TYPE_GPS) + "," + "LAT:" + String(gps.location.lat(), 6) + ",LON:" + String(gps.location.lng(), 6) + ",ALT:" + String(gps.altitude.meters()) + ",SPD:" + String(gps.speed.kmph()) + ",SAT:" + String(gps.satellites.value()) + ",TIME:" + timeString;
...
void sendActivityState(String state) {
  ...
  if (emergencyEventInProgress) {
    Serial.println("Skipping normal activity send during EMERGENCY.");
    return;
  }
  ...
  String accelState = String(MSG_TYPE_ACCEL) + "," + state + ",TIME:" + timeString;
...
void sendAccelData() {
  ...
  String accelData = String(MSG_TYPE_ACCEL) + "," + "AX:" + String(ax, 3) + ",AY:" + String(ay, 3) + ",AZ:" + String(az, 3) + ",GX:" + String(mpu6050.getGyroX(), 1) + ",GY:" + String(mpu6050.getGyroY(), 1) + ",GZ:" + String(mpu6050.getGyroZ(), 1) + ",ANX:" + String(mpu6050.getAngleX(), 1) + ",ANY:" + String(mpu6050.getAngleY(), 1) + ",ANZ:" + String(mpu6050.getAngleZ(), 1) + ",MAG:" + String(mag, 3) + ",ACTMAG:" + String(actMag, 3) + ",NOISE:" + String(noiseThreshold, 3) + ",STATE:" + currentActivity + ",TIME:" + timeString;
...
void sendEventData(String eventType) {
  ...
  if (gps.location.isValid()) {
    locationData = ",LAT:" + String(gps.location.lat(), 6) + ",LON:" + String(gps.location.lng(), 6);
  }
  String eventData = String(MSG_TYPE_EVENT) + "," + "TYPE:" + eventType + locationData + ",TIME:" + timeString;
...
void detectActivity(float activityMagnitude) {
  ...
  for (int i = 1; i < sampleCount; i++) {
    float prevMag = sqrt(accelBuffer[i - 1][0] * accelBuffer[i - 1][0] + accelBuffer[i - 1][1] * accelBuffer[i - 1][1] + accelBuffer[i - 1][2] * accelBuffer[i - 1][2]);
    float currMag = sqrt(accelBuffer[i][0] * accelBuffer[i][0] + accelBuffer[i][1] * accelBuffer[i][1] + accelBuffer[i][2] * accelBuffer[i][2]);

    if (abs(currMag - prevMag) > noiseThreshold * 1.5) {
      rhythmCount++;
    }
  }
  ...
  if (newActivity == lastActivity) {
    activityCounter++;
    if (activityCounter >= 2) {  // Changed from 5 to 2
...
void loop() {
  ...
  if (!gpsInStandby && !emergencyEventInProgress && currentActivity == "SLEEP" && (currentMillis - lastGPSActivity > GPS_STANDBY_TIMEOUT)) {
    setGPSStandby();
  }
  ...
  unsigned long gpsInterval = emergencyEventInProgress ? 500 : 1000;
